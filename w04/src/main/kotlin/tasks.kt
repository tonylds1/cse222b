...
class PharmaceuticalStockTracker {
    ...
    val count: Int
        get() {
            return inStockMedications.values.sumOf { it.size }
        }
    ...
    fun addContainer(container: MedicationContainer): Boolean {
        ...
        inStockMedications.getOrPut(container.ndcPackageCode) { mutableSetOf() }.add(container)
        return true
    }
}
 ...
 class LiquidMedicationContainer(
 ...
 ): MedicationContainer(ndcPackageCode, name, date) {

 }

 class TabletMedicationContainer(
     ndcPackageCode: String,
     name: String,
     date: Date,
     pillCount: Int,
...
fun PharmaceuticalStockTracker.removeExpired() {
    inStockMedications = inStockMedications.filterValues { containers ->
        containers.removeIf { it.isExpired }
...
val NDC_CODE_REGEX = Regex("\\d{5}-\\d{4}-\\d{2}")

fun isFormattedAsNDCCode(code: String): Boolean {
    // Replace the following line with your code
    return NDC_CODE_REGEX.matches(code)
}
...
fun PharmaceuticalStockTracker.addContainers(expectedNdcPackageCode: String, containersToAdd: Set<MedicationContainer>): AddMessage {
    return when {
        !isFormattedAsNDCCode(expectedNdcPackageCode) -> AddMessage.NDC_CODE_FORMAT_ERROR
        containersToAdd.isEmpty() -> AddMessage.EMPTY_CONTAINER_SET
        containersToAdd.any { it.ndcPackageCode != expectedNdcPackageCode } -> AddMessage.MIXED_NDC_CODES
        else -> {
            containersToAdd.forEach { addContainer(it) }
            AddMessage.SUCCESS
        }
    }
}
...
fun PharmaceuticalStockTracker.currentStock(of: String): Pair<StockMessage, List<MedicationContainer>> {
    return when {
        !isFormattedAsNDCCode(of) -> Pair(StockMessage.NDC_CODE_FORMAT_ERROR, listOf())
        inStockMedications[of] == null -> Pair(StockMessage.NO_INVENTORY, listOf())
        else -> Pair(StockMessage.SUCCESS, inStockMedications[of]!!.toList())
    }
}
...
fun PharmaceuticalStockTracker.sellContainers(count: Int, ndcPackageCode: String): Pair<SellMessage, List<MedicationContainer>> {
    return when {
        !isFormattedAsNDCCode(ndcPackageCode) -> Pair(SellMessage.NDC_CODE_FORMAT_ERROR, listOf())
        count <= 0 -> Pair(SellMessage.INVALID_COUNT, listOf())
        inStockMedications[ndcPackageCode] == null -> Pair(SellMessage.NO_INVENTORY, listOf())
        inStockMedications[ndcPackageCode]!!.size < count -> Pair(SellMessage.NOT_ENOUGH_INVENTORY, listOf())
        else -> {
            val stock = inStockMedications[ndcPackageCode]!!.sortedBy { it.expirationDate }
            val sold = stock.subList(0, count)
            inStockMedications[ndcPackageCode]!!.removeAll(sold)
            if (inStockMedications[ndcPackageCode]!!.isEmpty()) {
                inStockMedications.remove(ndcPackageCode)
            }
            Pair(SellMessage.SUCCESS, sold)
        }
    }
