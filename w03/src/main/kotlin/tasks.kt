...
open class MedicationContainer(
    var id: String = UUID.randomUUID().toString(),
    val name: String,
    val expirationDate: Date
) {
    constructor(name: String, expirationDate: Date): this(UUID.randomUUID().toString(), name, expirationDate)

    val isExpired: Boolean
        get() = expirationDate.before(Date())

    override fun toString(): String {
        return when (this) {
            is LiquidMedicationContainer -> "Liquid: $id"
            is TabletMedicationContainer -> "Tablet: $id"
            else -> "medical: $id"
        }
    }
...
class LiquidMedicationContainer(
    ...
    val concentrationUnits: String): MedicationContainer(name, expirationDate)
...
class TabletMedicationContainer(
    ...
    val pillCount: Int,
    val potency: Double,
    val potencyUnits: String
): MedicationContainer( name, expirationDate)

class PharmaceuticalStockTracker()
{
    val inStockMedications = mutableListOf<MedicationContainer>()
    ...
    fun count(name: String): Int {
        return inStockMedications.filter{ it.name == name }.count()
    }
}
...
fun task1(): Pair<LiquidMedicationContainer, TabletMedicationContainer>? {
    val liquid = LiquidMedicationContainer("liquid", futureDate(-1), 4.5, 2, "ml")
    var tablet = TabletMedicationContainer("tablet", futureDate(7), 90, 2.3, "mg")

    return Pair(liquid, tablet)
}
...
fun task4(int1: Int, int2: Int): Int {
    return when {
        task3(int1, int2) is IntRange -> 1
        task3(int1, int2) is List<*> -> 2
        else -> 0
    }
}
...
fun task5(anyArray: List<Any?>): List<Int> {
    return anyArray.mapNotNull {
        when (it) {
            is Int -> return@mapNotNull 1
            is Double -> return@mapNotNull 2
            is String -> return@mapNotNull 3
...
fun task6(anyArray: List<Any?>): List<Double> {
    return anyArray.mapNotNull {
        when (it) {
            is Int -> return@mapNotNull it.toDouble()
            is Double -> return@mapNotNull it
            is String -> return@mapNotNull it.toDoubleOrNull()
            else -> return@mapNotNull null
        }
    }
}
