...
fun task0(aBool: Boolean, int1: Int, int2: Int, aString: String): TestResult {
    if (aBool != true) return TestResult.INVALID_INPUT
    if (int1 < 0) return TestResult.INVALID_INPUT
    if (int2 <= int1) return TestResult.INVALID_INPUT
    if (aString.length != 4) return TestResult.INVALID_INPUT
...
fun task7(input: List<List<Int?>?>): Int {
    return input.sumOf { sublist ->
        sublist?.count { it == null } ?: 1
    }
}
